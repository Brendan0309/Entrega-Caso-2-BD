...
class DatabaseMigrator:
    ...
    def _extract_data(self):
        ...
        query = """
            SELECT planPriceid, amount, postTime, endDate, recurrencyType, `current`
            FROM Payment_PlanPrices
        """
    ...
    def _get_max_id(self):
        ...
        with self.sqlserver_conn.cursor() as cursor:
            cursor.execute("SELECT MAX(planPriceId) FROM caipiDb.dbo.caipi_planPrices")
    ...
    def _insert_data(self, df):
        ...
        with self.sqlserver_conn.cursor() as cursor:
            for _, row in df.iterrows():
                ...
                cursor.execute("""
                    INSERT INTO caipiDb.dbo.caipi_planPrices 
                    (amount, postTime, endDate, recurrencyType, [current], planId)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, row['amount'], row['postTime'], row['endDate'], row['recurrencyType'], row['current'], plan_id)
