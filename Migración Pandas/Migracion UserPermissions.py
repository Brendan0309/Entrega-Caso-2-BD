...
class DatabaseMigrator:
    ...
    def _extract_data(self):
        ...
        query = """
            SELECT rolePermissionid, enabled, deleted, lastUpdate, username, checksum, permissionid, userid
            FROM Payment_UserPermissions
        """
    ...
    def _get_max_id(self):
        ...
        with self.sqlserver_conn.cursor() as cursor:
            cursor.execute("SELECT MAX(userPermissionsId) FROM caipiDb.dbo.caipi_userPermissions")
    ...
    def _insert_data(self, df):
        ...
        with self.sqlserver_conn.cursor() as cursor:
            for _, row in df.iterrows():
                ...
                cursor.execute("""
                    INSERT INTO caipiDb.dbo.caipi_userPermissions 
                    (enabled, deleted, lastUpdate, username, checksum, permissionid, userid)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, row['enabled'], row['deleted'], row['lastUpdate'], row['username'], row['checksum'], row['permissionid'], row['userid'])
