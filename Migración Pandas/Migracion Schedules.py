...
class DatabaseMigrator:
    ...
    def _extract_data(self):
        ...
        query = """
            SELECT scheduleid, name, recurrencyType, `repeat`
    ...
    def _get_max_id(self):
        ...
        with self.sqlserver_conn.cursor() as cursor:
            cursor.execute("SELECT MAX(scheduleId) FROM caipiDb.dbo.caipi_Schedules")
    ...
    def _insert_data(self, df):
        ...
        with self.sqlserver_conn.cursor() as cursor:
            for _, row in df.iterrows():
                ...
                cursor.execute("""
                    INSERT INTO caipiDb.dbo.caipi_Schedules 
                    (name, recurrencyType, [repeat])
                    VALUES (?, ?, ?)
                """, row['name'], row['recurrencyType'], row['repeat'])
