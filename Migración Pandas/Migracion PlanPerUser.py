...
class DatabaseMigrator:
    ...
    def _extract_data(self):
        ...
        query = """
            SELECT planPerPersonid, adquisitionDate, expirationDate, enabled, userid 
            FROM Payment_PlanPerEntity
        """
    ...
    def _get_max_id(self):
        ...
        with self.sqlserver_conn.cursor() as cursor:
            cursor.execute("SELECT MAX(planPerUserId) FROM caipiDb.dbo.caipi_planPerUser")
    ...
    def _insert_data(self, df):
        ...
        with self.sqlserver_conn.cursor() as cursor:
            for _, row in df.iterrows():
                # Asignar un planId entre 1 y 7 (puedes ajustar la lógica según necesites)
                plan_id = (_ % 7) + 1  # Esto asignará ciclicamente 1-7 a cada registro
            
                cursor.execute("""
                    INSERT INTO caipiDb.dbo.caipi_planPerUser 
                    (planId, adquisitionDate, expirationDate, enabled, userid)
                    VALUES (?, ?, ?, ?, ?)
                """, plan_id, row['adquisitionDate'], row['expirationDate'], row['enabled'], row['userid'])
